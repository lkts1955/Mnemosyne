...
const NewMemory = () => {
  ...
  const [userFeeBack, setUserFeedBack] = useState("");
  ...
  const onSetImage = (event) => {
    ...
    // Ao selecionar umimage, execute aqui.....
    const files = event.target.files; //Pega os arquivos inseridos
    setImages(files);

    //Gerar as Thumbnails das imagens para monstra na tela
    for (let contador = 0; contador < files.length; contador++) {
      ...
      imageURLs.push(thumbnail); //acresta na lista
    }
  ...
  return (
    <>
      ...
      <main className="app-main">
        ...
        <p className="feedback">{userFeeBack}</p>
